...
class WidgetBorderHandler : public QObject, public WindowNativeEventFilter, public WindowSharedEventFilter
{
public:
    explicit WidgetBorderHandler(QWidget *widget, WindowContext *ctx, QObject *parent = nullptr)
    ...
    {
        if (isNormalWindow()) {
            widget->setContentsMargins({0, ctx->windowAttribute(QStringLiteral("border-thickness")).toInt(), 0, 0});
        } else {
    ...
    inline void resumeWidgetEventAndDraw(QWidget *w, QEvent *event)
    {
        ...
        ctx->virtual_hook(WindowContext::DrawWindows10BorderHook2, nullptr);
    }
