...
**/
#ifndef WINDOWKIT_WIN_P_H
#define WINDOWKIT_WIN_P_H
...
{
    return {margins.cxLeftWidth, margins.cyTopHeight, margins.cxRightWidth, margins.cyBottomHeight};
}
...
static inline bool isDarkWindowFrameEnabled(HWND hwnd)
{
    ...
    if (SUCCEEDED(apis.pDwmGetWindowAttribute(hwnd, _DWMWA_USE_IMMERSIVE_DARK_MODE, &enabled, sizeof(enabled)))) {
        return enabled;
    } else if (SUCCEEDED(apis.pDwmGetWindowAttribute(hwnd, _DWMWA_USE_IMMERSIVE_DARK_MODE_BEFORE_20H1, &enabled,
                                                     sizeof(enabled)))) {
        return enabled;
    }
    return false;
...
static inline quint32 getWindowFrameBorderThickness(HWND hwnd)
{
    const DynamicApis &apis = DynamicApis::instance();
    if (UINT result = 0;
        SUCCEEDED(apis.pDwmGetWindowAttribute(hwnd, _DWMWA_VISIBLE_FRAME_BORDER_THICKNESS, &result, sizeof(result)))) {
        return result;
    }
