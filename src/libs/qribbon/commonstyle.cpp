...
/* StyleHelper */
StyleHelper::StyleHelper(QObject *parent) : QObject(parent), m_pSettings(Q_NULL) {}

StyleHelper::~StyleHelper() { delete m_pSettings; }
...
/* CommonAnimation */
CommonAnimation::~CommonAnimation() { setWidget(Q_NULL); }

void CommonAnimation::setWidget(QWidget *widget)
{
    if (widget)
...
void CommonAnimation::paint(QPainter *painter, const QStyleOption *option)
{
    Q_UNUSED(option);
    Q_UNUSED(painter);
}

void CommonAnimation::drawBlendedImage(QPainter *painter, QRect rect, float alpha)
{
...
bool CommonAnimation::eventFilter(QObject *obj, QEvent *event)
{
...
CommonTransition::CommonTransition() : CommonAnimation() {}

CommonTransition::~CommonTransition() {}

void CommonTransition::paint(QPainter *painter, const QStyleOption *option)
{
    float alpha = 1.0;
    if (m_duration > 0) {
...
/*!
\class QRibbon::CommonPaintManager
\internal
*/
CommonPaintManager::CommonPaintManager(CommonStyle *baseStyle) : m_base(baseStyle) {}

CommonPaintManager::~CommonPaintManager() {}

CommonStyle *CommonPaintManager::baseStyle() const { return m_base; }

StyleHelper &CommonPaintManager::helper() const { return baseStyle()->helper(); }

QPixmap CommonPaintManager::cached(const QString &img) const { return baseStyle()->cached(img); }

QPixmap CommonPaintManager::cachedPath(const QString &img) const { return baseStyle()->CommonStyle::cached(img); }
...
void CommonPaintManager::drawImage(const QPixmap &dcSrc, QPainter &p, const QRect &rcDest, const QRect &rcSrc,
...
/*!
\class QRibbon::CommonStylePrivate
\internal
*/
...
{
    for (int i = m_animations.size() - 1; i >= 0; --i) {
        CommonAnimation *pItem = m_animations[i];
        ...
        if (!pWidget || !pWidget->isEnabled() || !pWidget->isVisible() || pWidget->window()->isMinimized() ||
            !pItem->running()) {
            CommonAnimation *a = m_animations.takeAt(i);
            delete a;
        }
    }
...
CommonAnimation *CommonStylePrivate::widgetAnimation(const QWidget *widget) const
{
    if (!widget)
        return 0;
    foreach(CommonAnimation * a, m_animations)
    {
        if (a->widget() == widget)
            return a;
    }
...
void CommonStylePrivate::setPalette(QWidget *widget) { m_customPaletteWidgets.insert(widget, widget->palette()); }
...
/*! \reimp */
void CommonStyle::drawPrimitive(PrimitiveElement pe, const QStyleOption *option, QPainter *p,
...
/*! \reimp */
void CommonStyle::drawComplexControl(ComplexControl cc, const QStyleOptionComplex *opt, QPainter *p,
                                     const QWidget *widget) const
{
    bool ret = false;
    bool qtcStyle = false;
...
/*! \reimp */
QSize CommonStyle::sizeFromContents(ContentsType ct, const QStyleOption *opt, const QSize &contentsSize,
...
/*! \reimp */
QRect CommonStyle::subControlRect(ComplexControl cc, const QStyleOptionComplex *opt, SubControl sc,
                                  const QWidget *widget) const
{
#if (QT_VERSION >= QT_VERSION_CHECK(5, 0, 0))
    #ifdef Q_OS_WIN
...
/*! \reimp */
int CommonStyle::styleHint(StyleHint hint, const QStyleOption *opt, const QWidget *widget,
...
/*! \internal */
QPixmap CommonStyle::cached(const QString &img) const { return ::cachedPixmap(img); }

/*! \internal */
void CommonStyle::clearCache() { QPixmapCache::clear(); }

/*! \internal */
void CommonStyle::drawImagePart(const QPixmap &soSrc, QPainter &p, const QRect &rcDest, const QRect &rcSrc,
                                bool alpha) const
{
    Q_UNUSED(alpha);
...
/*! \internal */
void CommonStyle::drawPixmap(const QPixmap &soSrc, QPainter &p, const QRect &rcDest, const QRect &rcSrc, bool alpha,
                             QRect rcSizingMargins, bool alphaBlend) const
{
    Q_UNUSED(alphaBlend)
    QPixmap dcSrc = soSrc;
...
bool CommonStyle::transitionsEnabled() const { return false; }
                                 ...
                                 const QWidget *widget, int iDurationOver) const
{
    ...
    if (widget && !wasDetermined && bAnimation) {
        ...
        if (doTransition) {
            ...
            CommonAnimation *anim = d->widgetAnimation(widget);

            startImage.fill(0);
            CommonTransition *transition = new CommonTransition;
            ...
            } else if (const QStyleOptionToolButton *button =
            ...
            } else if (const StyleRibbonOptionHeader *optHendle =
                           qstyleoption_cast<const StyleRibbonOptionHeader *>(option)) {
            ...
            } else if (const QStyleOptionComplex *pOptionComplex =
                           qstyleoption_cast<const QStyleOptionComplex *>(option)) {
