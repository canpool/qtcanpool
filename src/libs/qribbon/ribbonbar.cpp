...
/*!
 */
LogotypeLabel::LogotypeLabel(QWidget *parent) : QLabel(parent), m_alignment(Qt::AlignRight) {}
...
RibbonBarPrivate::~RibbonBarPrivate() { removeAllContextHeaders(); }
...
void RibbonBarPrivate::init()
{
    ...
    if (ribbonStyle)
        ...
        q->setFont(QApplication::font("QRibbon::RibbonBar"));
...
void RibbonBarPrivate::layoutRibbon()
{
    ...
    if (q->tabBarPosition() == RibbonBar::TabBarBottomPosition) {
    ...
    if (m_controlSystemButton) {
        ...
        int add = q->tabBarPosition() == RibbonBar::TabBarBottomPosition ? 2 : 0;
    ...
    bool quickAccessNearTabs =
        m_quickAccessBar->isVisible() && m_quickAccessBarPosition == RibbonBar::QATopPosition && !showTitle;
...
QAction *RibbonBarPrivate::createSystemButton(const QIcon &icon, const QString &text)
{
    ...
    m_controlSystemButton = new RibbonSystemButton(q);
...
int RibbonBarPrivate::indexPage(RibbonPage *page) const { return m_listPages.indexOf(page); }

int RibbonBarPrivate::heightTabs() const { return m_heightTabs; }
...
void RibbonBarPrivate::currentChanged(int index)
{
    ...
    if (validIndex(index)) {
        if (!m_minimized) {
            ...
            RibbonPage *page = q->getPage(index);
            page->updateLayout();

            if (updates)
                q->setUpdatesEnabled(updates);
        } else {
            if (RibbonPage *page = q->getPage(index)) {
                emit q->currentPageIndexChanged(index);
                emit q->currentPageChanged(m_listPages[index]);
...
void RibbonBarPrivate::titleBarOptions(StyleOptionTitleBar &titleBarOptions) const
{
    ...
    titleBarOptions.quickAccessVisible =
        (m_quickAccessBar != Q_NULL) && m_quickAccessBar->isVisible() && !szAccessBar.isNull();
...
QAction *RibbonBarPrivate::addTitleButton(QStyle::SubControl subControl, bool add, QRect &rcControl)
{
    ...
    if (!but && add) {
        act = new QAction(q);
        but = new RibbonTitleButton(q, subControl);
        but->show();
...
QMenu *RibbonBarPrivate::createContextMenu()
{
    ...
    if (q->isQuickAccessVisible()) {
        if (RibbonQuickAccessBar *quickAccessBar = q->quickAccessBar()) {
            QList<QAction *> actions = quickAccessBar->actions();
            if (actions.size() > 0) {
                action = popup->addAction(RibbonBar::tr_compatible(RibbonCustomizeQuickAccessToolBarDotString));
                action->setObjectName(strCustomizeQAToolBar);
                connect(action, SIGNAL(triggered()), this, SLOT(toggledCustomizeBar()));

                action = popup->addAction(q->quickAccessBarPosition() == RibbonBar::QATopPosition
                                              ? RibbonBar::tr_compatible(RibbonShowQuickAccessToolBarBelowString)
                                              : RibbonBar::tr_compatible(RibbonShowQuickAccessToolBarAboveString));
    ...
    action = popup->addAction(RibbonBar::tr_compatible(RibbonCustomizeActionString));
    action->setObjectName(strCustomizeRibbonBar);
    connect(action, SIGNAL(triggered()), this, SLOT(toggledCustomizeBar()));

    if (q->isMinimizationEnabled()) {
        action = popup->addAction(RibbonBar::tr_compatible(RibbonMinimizeActionString));
...
void RibbonBarPrivate::createKeyTips()
{
    ...
    for (int i = 0, count = list.count(); i < count; i++) {
        ...
        if (!button->isVisible() || button->objectName() == QLatin1String("RibbonQuickAccessButton"))
            continue;
        ...
        m_keyTips.append(
            new RibbonKeyTip(q, button, strCaption,
    ...
    if (m_tabBar && m_tabBar->isVisible()) {
        for (int i = 0, count = m_tabBar->getTabCount(); i < count; i++) {
            ...
            if (!strCaption.isEmpty() && tab->isEnabled() && tab->isVisible()) {
                ...
                RibbonKeyTip *keyTip = new RibbonKeyTip(q, tab, strCaption, tab->mapToGlobal(pt),
    ...
    if (m_controlSystemButton) {
        ...
        m_keyTips.append(new RibbonKeyTip(q, m_controlSystemButton, strCaption, m_controlSystemButton->mapToGlobal(pt),
                                          RibbonKeyTip::AlignTipCenter | RibbonKeyTip::AlignTipVCenter,
                                           ...
                                           const QRect &rcGroups, bool visible)
{
    ...
    const int heightCaptionGroup =
        q->style()->pixelMetric((QStyle::PixelMetric)RibbonStyle::PM_RibbonHeightCaptionGroup, &opt, group);
    ...
    RibbonKeyTip *keyTip = new RibbonKeyTip(q, widget, strCaption, pt, align, widget->isEnabled());
...
bool RibbonBarPrivate::createPopupKeyTips(QMenu *levalMenu)
{
    ...
    if (menu) {
        ...
        for (int i = 0; i < list.count(); i++) {
            ...
            RibbonKeyTip *keyTip = new RibbonKeyTip(q, menu, strCaption, menu->mapToGlobal(pt),
                                                    RibbonKeyTip::AlignTipTop, action->isEnabled(), action);
        ...
        for (int i = 0; i < listWidget.count(); i++) {
            ...
            RibbonKeyTip *keyTip = new RibbonKeyTip(q, widget, strCaption, widget->mapToGlobal(pt),
...
void RibbonBarPrivate::calcPositionKeyTips()
{
    for (int i = 0, count = m_keyTips.count(); i < count; i++) {
        ...
        QRect rc(QPoint(pt.x() - ((flagAlign & RibbonKeyTip::AlignTipCenter)  ? sz.width() * 0.5
                                  : (flagAlign & RibbonKeyTip::AlignTipRight) ? sz.width()
                                                                              : 0),
                        pt.y() - (flagAlign & RibbonKeyTip::AlignTipVCenter  ? sz.height() * 0.5
                                  : flagAlign & RibbonKeyTip::AlignTipBottom ? sz.height()
                                                                             : 0)),
                 sz);
...
int RibbonBarPrivate::rowItemHeight() const { return m_rowItemHeight; }

int RibbonBarPrivate::rowItemCount() const { return m_rowItemCount; }
...
int RibbonBarPrivate::maxGroupHeight() const
{
    ...
    QStyleOption opt;
    opt.init(q);
    ret += q->style()->pixelMetric((QStyle::PixelMetric)RibbonStyle::PM_RibbonHeightCaptionGroup, &opt, Q_NULL) + 4;
                    ...
                    if (QAbstractButton *absButton = qobject_cast<QAbstractButton *>(keyTip->getOwner())) {
                        ...
                        if (QAction *action = keyTip->getAction()) {
                            if (qobject_cast<RibbonPageSystemPopup *>(action->menu())) {
                                if (QMenu *activPopup =
                                        qobject_cast<RibbonSystemPopupBar *>(QApplication::activePopupWidget()))
...
void RibbonBarPrivate::createCustomizeDialog()
{
    ...
    m_customizeDialog = new RibbonCustomizeDialog(q->parentWidget());
    // Create "Customize QAT" page:
    RibbonQuickAccessBarCustomizePage *quickAccessBarCustomizePage = new RibbonQuickAccessBarCustomizePage(q);
    m_customizeDialog->addPage(quickAccessBarCustomizePage);
    // Create "Customize Ribbon" page:
    RibbonBarCustomizePage *ribbonBarCustomizePage = new RibbonBarCustomizePage(q);
...
RibbonPage *RibbonBar::addPage(const QString &text) { return insertPage(-1, text); }

void RibbonBar::addPage(RibbonPage *page) { insertPage(-1, page); }
...
/*!
\brief The function sets the policy of assignment for KeyTips of the buttons and other controls. If the parameter \a
complement is true then the keytips assigned automatically, based on keyboard shortcuts <&File, &Open...>. If parameter
\a complement is false keytips will be assigned only if user set it by RibbonBar::setKeyTip() function. \sa setKeyTip()
*/
...
/*!
\brief Sets the \a keyTip for the control associated with given \a action. Function is taken effect if
RibbonBar::isKeyTipsComplement() return true. \sa setKeyTipsComplement()
*/
...
*/
void RibbonBar::minimize() { setMinimizedFlag(true); }
...
*/
void RibbonBar::setMinimized(bool flag) { setMinimizedFlag(flag); }

/*!
Do maximize the ribbon bar. This is equivalent of setMaximized(true) or setMinimized(false) call.
*/
void RibbonBar::maximize() { setMinimizedFlag(false); }

/*!
Returns true if ribbon bar is maximized. Otherwise returns false.
*/
bool RibbonBar::isMaximized() const { return !isMinimized(); }

/*!
Sets maximized \a flag for the ribbon bar.
*/
void RibbonBar::setMaximized(bool flag) { setMinimizedFlag(!flag); }
...
/*!
Shows customization dialog related to customizeDialog() function. Dialog is shown in modal mode to window that holds a
ribbon bar. \sa customizeDialog
*/
...
QAction *RibbonBar::addSystemButton(const QString &text) { return addSystemButton(QIcon(), text); }
...
This property holds whether display the modern Office 2007, 2010, 2013 frame and activate Windows Air support
(applications under Windows 8, 7,Vista) is enabled. By default parameter is enabled. Used on Windows platform only.
\inmodule ribbon
*/
...
/*! \reimp */
void RibbonBar::changeEvent(QEvent *event) { return QMenuBar::changeEvent(event); }
...
/*!
Installs translator to QApplication based on \a country variable. Format variable is de_de, ru_ru, fr_fr. If \a country
is empty then loaded QTranslator based on current locale setting.
*/
...
{
#if (QT_VERSION >= QT_VERSION_CHECK(5, 0, 0))
    return QCoreApplication::translate("QRibbon::RibbonBar", s, c);
#else
    return QCoreApplication::translate("QRibbon::RibbonBar", s, c, QCoreApplication::CodecForTr);
#endif
}
