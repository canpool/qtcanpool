...
void ManagerPopup::appendPopup(OfficePopupWindow *popup) { m_listPopup.append(popup); }

void ManagerPopup::removePopup(OfficePopupWindow *popup) { m_listPopup.removeOne(popup); }
...
void OfficePopupWindowPrivate::handleMouseMoveEvent(QMouseEvent *event)
{
    ...
    if (m_dragging && m_form) {
        ...
        ManagerPopup::getMngPopup().m_ptPopup = q->getPosition();
        return;
    }
...
PopupState OfficePopupWindowPrivate::getPopupState() const { return m_popupState; }
...
void OfficePopupWindowPrivate::createStyle()
{
    ...
    if (OfficeStyle *officeStyle = qobject_cast<OfficeStyle *>(q->style()))
        ...
        q->setStyle(new PopupDrawHelper());
}
...
/*!
    Static method to quick show the office popup box with given \a parent, \a titleIcon - title icon, \a titleText -
   title text and bodyText - body text. You may use HTML to fill \a titleText and \a bodyText. Optional parameters \a
   closeButtonVisible and \a decoration are used for the customization purposes. After it will be closed the instance of
   the window will be deleted automaticaly.

    \image ribbonui_officepopupwindow5.png
    To show such message on the desktop you need invoke code bellow:

    \code
    QString titleHtml = "<span style=\"font-family:'MS Shell Dlg 2'; font-size:8pt; font-weight:600;
   font-style:normal;\">John Smith</span><br>Re: Your request";
    QString bodyHtml = "<a href='#'><img src=':/res/cross.ico'></a> \
                    <a href='#'><img src=':/res/flag.ico'></a> \
                    <span style=\"color:#0000ff; font-family:'MS Shell Dlg 2'; font-size:8pt; font-weight:400;
   font-style:normal;\"> \ I am sending you the requested inform&nbsp;&nbsp;&nbsp;</span>";
...
The method is used when you have to show the non-standard contents inside the popup box. In this case the settings of
the titleText(), titleIcon() and bodyText() will be ignored and given \a widget will be showing instead.
*/
...
/*!
Shows popup box on the desktop at the given \a pos. This method is equal to: setPosition(pos); showPopup(); line of
code.
*/
...
/*!
\brief Sets the drag-and-drop \a enabled flag for the popup box. If \a enabled is true then the user may use grip to
drag-and-drop window over desktop.
*/
...
/*!
\brief Returns the drag-and-drop flag for the popup box. If returned value is true then the user may use grip to
drag-and-drop window over desktop.
*/
...
static void setChildStyle(QWidget *widget, QStyle *style)
{
    ...
    foreach(QWidget * w, widgets)::setChildStyle(w, style);
}
