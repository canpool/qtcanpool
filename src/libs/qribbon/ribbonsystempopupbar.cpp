...
class QtcEventParent : public QEvent
{
public:
    QtcEventParent(QWidget *parent) : QEvent(QEvent::ParentChange) { m_parent = parent; }
    QWidget *m_parent;
};
...
QColor RibbonSystemButton::backgroundColor() const { return palette().color(QPalette::Button); }
...
SysPopupRibbonButton::SysPopupRibbonButton(QWidget *parent) : QToolButton(parent) {}
...
RibbonPageSystemPopup *RibbonSystemPopupBar::addPageSystemPopup(const QString &caption, QAction *defaultAction,
                                                                bool splitAction)
{
...
void RibbonSystemPopupBar::paintEvent(QPaintEvent *event)
{
    ...
    if (count > 0) {
        ...
        const QColor fillColor =
            static_cast<QRgb>(style()->styleHint(QStyle::SH_CustomBase, Q_NULL, Q_NULL, &hintReturn));
        ...
        const QColor rectColor =
            static_cast<QRgb>(style()->styleHint(QStyle::SH_CustomBase, Q_NULL, Q_NULL, &hintRectReturn));
    ...
    for (int i = 0; i < count; i++) {
        if (RibbonPageSystemPopupListCaption *page =
            ...
            const QColor rectColor =
                static_cast<QRgb>(style()->styleHint(QStyle::SH_CustomBase, Q_NULL, Q_NULL, &hintColorReturn));
...
RibbonPageSystemPopupListCaption::RibbonPageSystemPopupListCaption(const QString &caption) { setWindowTitle(caption); }
...
void RibbonPageSystemPopupListCaption::paintEvent(QPaintEvent *event)
{
    ...
    const QColor colorClientText =
        static_cast<QRgb>(style()->styleHint(QStyle::SH_CustomBase, Q_NULL, Q_NULL, &hintClientText));
    ...
    const QColor fillEdgeShadow =
        static_cast<QRgb>(style()->styleHint(QStyle::SH_CustomBase, Q_NULL, Q_NULL, &hintEdgeShadow));
    ...
    const QColor edgeHighLightColor =
        static_cast<QRgb>(style()->styleHint(QStyle::SH_CustomBase, Q_NULL, Q_NULL, &hintEdgeHighLight));
...
void RibbonPageSystemRecentFileList::drawEvent(QPaintEvent *event, QPainter *p)
{
    ...
    for (int i = 0; i < d->m_recentFileActs.count(); ++i) {
        ...
        const QColor clientTextColor =
            static_cast<QRgb>(style()->styleHint(QStyle::SH_CustomBase, Q_NULL, Q_NULL, &hintClientTextReturn));
...
bool RibbonPageSystemPopup::event(QEvent *event)
{
    ...
    if (event->type() == QEvent::Move) {
        ...
        if (QtcEventParent *e = dynamic_cast<QtcEventParent *>(event)) {
            if (!d->m_parentMenu) {
...
void RibbonPageSystemPopup::leaveEvent(QEvent *event) { Q_UNUSED(event); }
