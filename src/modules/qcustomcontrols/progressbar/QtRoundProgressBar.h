 ...
 **
****************************************************************************/
#ifndef QTROUNDPROGRESSBAR_H
#define QTROUNDPROGRESSBAR_H

#include <QWidget>

/**
 * @brief The QtRoundProgressBar class represents a circular progress bar and maintains its API
 * similar to the *QProgressBar*.
 *
 * ### Styles
 * QtRoundProgressBar currently supports Donut, Pie and Line styles. See setBarStyle() for more details.
 *
 * ### Colors
 * Generally QtRoundProgressBar uses its palette and font attributes to define how it will look.
 *
 ...
 *
 * ### Value text
 * Value text is generally drawn inside the QtRoundProgressBar using its `font()` and \a QPalette::Text role from its `palette()`.
 *
 ...
 */
class QtRoundProgressBar : public QWidget
{
    Q_OBJECT
public:    
    explicit QtRoundProgressBar(QWidget *parent = nullptr);
